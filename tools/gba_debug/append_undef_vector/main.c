...
//---------------------------------------------------------------------------------
{
    ...
    int i;
    for(i = 0; i < 8; i++) addr[i] = asciitoint(addr[i]);
    unsigned int address = 0;
    for(i = 0; i < 8; i++) address = (16*address) + addr[i];
    fclose(faddr);
    printf("\nRead address: 0x%08X\n",address);
    ...
    char c = 0;
    while(add_size--) fwrite(&c,1,1,datafile);

    unsigned char op[4] = { 0x00, 0x00, 0x1F, 0xE5} ;//ldr r0,=(FUNC_ADDR|1)
    fwrite(&op,1,4,datafile);
    unsigned char op2[4] = { 0x10, 0xFF, 0x2F, 0xE1} ;//bx r0
    fwrite(&op2,1,4,datafile);

    int UndefVector = address | 1;
    unsigned char op3[4] = { UndefVector&0xFF,
                             (UndefVector>>8)&0xFF,
                             (UndefVector>>16)&0xFF,
                             (UndefVector>>24)&0xFF } ; //(FUNC_ADDR|1) -- switch to thumb
