...
static _gui_element gb_mapview_tilebase8000_radbtn,
                    gb_mapview_tilebase8800_radbtn;
static _gui_element gb_mapview_mapbase9800_radbtn,
                    gb_mapview_mapbase9C00_radbtn;

static _gui_element gb_mapview_zoomed_tile_bmp;
static _gui_element gb_mapview_zoomed_tile_bw_radbtn,
...
void Win_GBMapViewerUpdate(void)
{
    ...
    u8 *bgtilemap = gb_mapview_selected_mapbase ?
    ...
    {
        GB_Debug_MapPrint(gb_map_buffer, GB_MAP_BUFFER_WIDTH,
                          GB_MAP_BUFFER_HEIGHT, gb_mapview_selected_mapbase,
                          gb_mapview_selected_tilebase);
    }
    ...
    {
        GB_Debug_TileDrawZoomedPaletted64x64(gb_map_zoomed_tile_buffer, tile,
                                             bank, info & 7, 0);
    }
}
...
static void _win_gb_mapviewer_dump_btn_callback(void)
{
    ...
    Save_PNG(name, gb_map_buffer, GB_MAP_BUFFER_WIDTH, GB_MAP_BUFFER_HEIGHT, 0);
...
int Win_GBMapViewerCreate(void)
{
    ...
    GUI_SetRadioButton(&gb_mapview_tilebase8000_radbtn,
                       6, 24, 8 * FONT_WIDTH, 24,
                       "0x8000", 0, 0, 1,
                       _win_gb_map_viewer_tilebase_radbtn_callback);
    GUI_SetRadioButton(&gb_mapview_tilebase8800_radbtn,
                       12 + 8 * FONT_WIDTH, 24, 8 * FONT_WIDTH, 24,
                       "0x8800", 0, 1, 0,
                       _win_gb_map_viewer_tilebase_radbtn_callback);

    GUI_SetLabel(&gb_mapview_mapbase_label, 6, 54, 8 * FONT_WIDTH, FONT_HEIGHT,
                 "Map Base");

    GUI_SetRadioButton(&gb_mapview_mapbase9800_radbtn,
                       6, 72, 8 * FONT_WIDTH, 24,
                       "0x9800", 1, 0, 1,
                       _win_gb_map_viewer_mapbase_radbtn_callback);
    GUI_SetRadioButton(&gb_mapview_mapbase9C00_radbtn,
                       12 + 8 * FONT_WIDTH, 72, 8 * FONT_WIDTH, 24,
                       "0x9C00", 1, 1, 0,
                       _win_gb_map_viewer_mapbase_radbtn_callback);
    ...
    GUI_SetBitmap(&gb_mapview_map_bmp, 145, 6,
                  GB_MAP_BUFFER_WIDTH, GB_MAP_BUFFER_HEIGHT, gb_map_buffer,
                  _win_gb_mapviewer_map_bmp_callback);

    GUI_SetBitmap(&gb_mapview_zoomed_tile_bmp, 6, 198, 64, 64,
                  gb_map_zoomed_tile_buffer, NULL);

    GUI_SetRadioButton(&gb_mapview_zoomed_tile_bw_radbtn,
                       76, 198, 4 * FONT_WIDTH, 18,
                       "B/W", 2, 0, 0,
                       _win_gb_mapviewer_zoomed_tile_bw_radbtn_callback);
    GUI_SetRadioButton(&gb_mapview_zoomed_tile_pal_radbtn,
                       76 + 4 * FONT_WIDTH + 6, 198, 4 * FONT_WIDTH, 18,
                       "Pal", 2, 1, 1,
                       _win_gb_mapviewer_zoomed_tile_bw_radbtn_callback);

    GUI_SetButton(&gb_mapview_dumpbtn, 87, 238, FONT_WIDTH * 6, FONT_HEIGHT * 2,
                  "Dump", _win_gb_mapviewer_dump_btn_callback);
    ...
    gb_map_zoomed_tile_is_pal = 1;

    WinIDGBMapViewer = WH_Create(WIN_GB_MAPVIEWER_WIDTH,
                                 WIN_GB_MAPVIEWER_HEIGHT, 0, 0, 0);
