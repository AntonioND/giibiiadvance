...
static _gui_element gb_tileview_zoomed_tile_bw_radbtn,
                    gb_tileview_zoomed_tile_pal_bg_radbtn,
                    gb_tileview_zoomed_tile_pal_spr_radbtn;
...
void Win_GBTileViewerUpdate(void)
{
    ...
    if (gb_tile_zoomed_tile_is_pal == 0)
    {
        GB_Debug_TileVRAMDraw(gb_tile_bank0_buffer,
                              GB_TILE_BUFFER_WIDTH, GB_TILE_BUFFER_HEIGHT,
                              gb_tile_bank1_buffer,
                              GB_TILE_BUFFER_WIDTH, GB_TILE_BUFFER_HEIGHT);
    }
    else
    {
        GB_Debug_TileVRAMDrawPaletted(gb_tile_bank0_buffer,
                                      GB_TILE_BUFFER_WIDTH,
                                      GB_TILE_BUFFER_HEIGHT,
                                      gb_tile_bank1_buffer,
                                      GB_TILE_BUFFER_WIDTH,
                                      GB_TILE_BUFFER_HEIGHT,
    ...
    if (gb_tile_zoomed_tile_is_pal == 0)
    {
        GB_Debug_TileDrawZoomed64x64(gb_tile_zoomed_tile_buffer,
                                     gb_tileview_selected_index,
                                     gb_tileview_selected_bank);
    }
    else
    {
        GB_Debug_TileDrawZoomedPaletted64x64(gb_tile_zoomed_tile_buffer,
                                             gb_tileview_selected_index,
                                             gb_tileview_selected_bank,
                                             gb_tile_zoomed_tile_sel_pal,
                                             gb_tile_zoomed_tile_is_pal == 2);
    ...
    {
        GUI_ConsoleModePrintf(&gb_tileview_con, 0, 0,
                              "Tile: %d(%d)\n"
                              "Addr: 0x%04X\n"
                              "Bank: %d",
                              tile, tileindex, 0x8000 + (tile * 16),
                              gb_tileview_selected_bank);
    }
    else
    {
...
int Win_GBTileViewerCreate(void)
{
    ...
    GUI_SetLabel(&gb_tileview_bank1_label,
                 251, 6, GB_TILE_BUFFER_WIDTH, FONT_HEIGHT, "Bank 1");

    GUI_SetTextBox(&gb_tileview_textbox, &gb_tileview_con,
                   6, 6, 15 * FONT_WIDTH, 3 * FONT_HEIGHT, NULL);

    GUI_SetBitmap(&gb_tileview_bank0_bmp,
                  117, 24, GB_TILE_BUFFER_WIDTH, GB_TILE_BUFFER_HEIGHT,
                  gb_tile_bank0_buffer, _win_gb_tileviewer_bank0_bmp_callback);
    GUI_SetBitmap(&gb_tileview_bank1_bmp,
                  251, 24, GB_TILE_BUFFER_WIDTH, GB_TILE_BUFFER_HEIGHT,
                  gb_tile_bank1_buffer, _win_gb_tileviewer_bank1_bmp_callback);

    GUI_SetBitmap(&gb_tileview_zoomed_tile_bmp, 6, 48, 64, 64,
                  gb_tile_zoomed_tile_buffer, NULL);

    GUI_SetRadioButton(&gb_tileview_zoomed_tile_bw_radbtn,
                       6, 118, 4 * FONT_WIDTH, 18,
                       "B/W", 0, 0, 0,
    ...
    GUI_SetScrollBar(&gb_tileview_zoomed_tile_pal_scrollbar,
                     6, 142, 96, 12,
                     0, 7, 0, _win_gb_tileviewer_pal_select_scrollbar_callback);
    GUI_SetLabel(&gb_tileview_zoomed_tile_pal_label,
                 6, 160, 6 * FONT_WIDTH, FONT_HEIGHT, "Pal: -");

    GUI_SetButton(&gb_tileview_dumpbtn,
                  6, 192, FONT_WIDTH * 6, FONT_HEIGHT * 2, "Dump",
    ...
    WinIDGBTileViewer = WH_Create(WIN_GB_TILEVIEWER_WIDTH,
                                  WIN_GB_TILEVIEWER_HEIGHT, 0, 0, 0);
