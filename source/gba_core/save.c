...
void GBA_DetectSaveType(u8 *romptr, int size)
{
    for (int j = 0; j < SAV_TYPES; j++)
    {
        for (int i = 0; i < size; i++)
        {
            ...
            if (i + len < size)
            {
                if (str_cmp_len((const char *)&(romptr[i]),
                                save_type_strings[j], len))
                {
    ...
    // Not detected any string... Try to autodetect save type.
...
// 0 if nothing, 1 if 5555=0xAA, 2 if 2AAA=0x55 (ready for command)
u32 FLASH_CMD_STATE;

int eeprom_detect_size;
u64 EEPROM_BUFFER[1024];
u32 EEPROM_SIZE;
u32 EEPROM_ADDRESS_BUS;
u32 EEPROM_ADDRESS;
u32 EEPROM_ADDRESS_MASK;
u32 EEPROM_CMD;
u32 EEPROM_CMD_LEN;
                ...
                switch (FLASH_CMD)
                {
                    ...
                    case 0x90: // (ID mode)
                    {
                        if (address == 0x0E000000)
                            return 0x32; // Device
                        if (address == 0x0E000001)
                            return 0x1B; // Manufacturer
                        break;
                    }
                    default:
                        break;
    ...
    switch (SAVE_TYPE)
    {
        case SAV_EEPROM:
        {
            if (gba_dma3enabled())
            {
                ...
                if (EEPROM_CMD == 3)
                {
                    if ((EEPROM_DATA_STREAMING < 69)
                        && (EEPROM_DATA_STREAMING > 64))
                    {
            ...
            {
                //Debug_DebugMsgArg("EEPROM: Read halfword from [%08X]",
                //                  address);
                return 1; // Ready
            }
...
void GBA_SaveWrite8(u32 address, u8 data)
{
    if (SAVE_TYPE == SAV_AUTODETECT)
    {
        //Debug_DebugMsgArg("Autodetect: Write 8bit [%08X]=%02X", address,
        //                  data);

        if ((address == 0x0E005555) && (data == 0xAA))
        {
            // Default to this rather than 512 in case it switches banks
            SAVE_TYPE = SAV_FLASH1M;
    ...
    switch (SAVE_TYPE)
    {
        ...
        case SAV_FLASH512:
        {
            if (FLASH_CMD_STATE < 2)
            {
                ...
                {
                    if ((data == 0xAA) && (FLASH_CMD_STATE == 0))
                        FLASH_CMD_STATE = 1;
                    else
                        FLASH_CMD_STATE = 0;
                }
                ...
                {
                    if ((FLASH_CMD_STATE == 1) && (data == 0x55))
                        FLASH_CMD_STATE = 2;
                    else
                        FLASH_CMD_STATE = 0;
                }
            ...
            else if (FLASH_CMD_STATE == 2)
            {
                if (((address & 0xFF000000) == 0x0E000000)
                    && ((address & 0x00FF0FFF) == 0) && (data == 0x30))
                {
                    // Erase sector
                    if (FLASH_CMD == 0x80)
                    {
                ...
                else if (address == 0x0E005555) // Execute command
                {
                    ...
                    switch (data)
                    {
                        case 0x10: // After 80h -- (erase entire chip)
                            if (FLASH_CMD == 0x80)
                            {
                                memset(FLASH_BUFFER512, 0xFF,
                                       sizeof(FLASH_BUFFER512));
                                FLASH_CMD = 0x10;
                                FLASH_STATE = 1;
                            }
                            else
                ...
                {
                    FLASH_CMD_STATE = 0; // Error?
                }
            }
            else if (FLASH_CMD_STATE == 3)
            {
                // For commands that need another instruction
        ...
        case SAV_FLASH1M:
        {
                ...
                else if (address == 0x0E005555) // Execute command
                {
                    ...
                    switch (data)
                    {
                        case 0x10: // After 80h -- (erase entire chip)
                            if (FLASH_CMD == 0x80)
                            {
                                memset(FLASH_BUFFER1M, 0xFF,
                                       sizeof(FLASH_BUFFER1M));
                                FLASH_CMD = 0x10;
                                FLASH_STATE = 1;
                            }
                            else
            ...
            else if (FLASH_CMD_STATE == 3)
            {
                ...
                else if (FLASH_CMD == 0xB0) // Change bank
                {
                    ...
                    {
                        FLASH_1M_PTR = (data & 1) ?
                            &(FLASH_BUFFER1M[0x10000]) : FLASH_BUFFER1M;
                    }
                }
                FLASH_CMD_STATE = 0;
                FLASH_CMD = 0;
        ...
        case SAV_EEPROM:
        {
            if (gba_dma3enabled())
            {
                if (eeprom_detect_size)
                {
                    ...
                    {
                        Debug_ErrorMsgArg("EEPROM: Failed size autodetection. %d",
                                          gba_dma3numchunks());
                ...
                //Debug_DebugMsgArg("EEPROM: Write 16bit [%08X]=%04X", address,
                //                  data);
...
void GBA_SaveReadFile(void)
{
    switch (SAVE_TYPE)
    {
        case SAV_SRAM:
        {
            ...
            if (f == NULL) // Maybe file didn't exist
                return;
        ...
        case SAV_FLASH1M:
        {
            ...
            {
                //Debug_ErrorMsgArg("Couldn't read data from file.");
                // TODO: Maybe save type autodetection said it is FLASH1M , but
                // it is FLASH512, the save is FLASH512 and tried to read 1M...
            }
            fclose(f);
            return;
        }
        case SAV_EEPROM:
        {
            ...
            if ((size != 512) && (size != 8 * 1024))
                Debug_ErrorMsgArg("Corrupted saved data (wrong size).");
