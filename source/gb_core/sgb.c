...
//void SGB_Clock(int clocks)  // for delay between frames (not used for now)
//{
...
static void SGB_ATTR_BLK(void)
{
    ...
    //    Debug_DebugMsgArg("B:%d P:%d 1:%d,%d 2:%d,%d",curbyte,curpacket,x1,y1,x2,y2);
...
static void SGB_DATA_SND(void)
{
    ...
    {
        Debug_DebugMsgArg("SGB DATA_SND warning - Wrote to bank %02xh",SGBInfo.data[0][3]);
        return;
    }
}
...
static void SGB_ExecuteCommand(void)
{
    ...
    Debug_DebugMsgArg("Unknown command: %02x, Packets: %02x",SGBInfo.data[0][0] >> 3, SGBInfo.data[0][0]&0x07);
    u32 i;
    for(i = 0; i < (SGBInfo.data[0][0]&0x07);i++)
    {
        Debug_DebugMsgArg("%02x%02x %02x%02x %02x%02x %02x%02x %02x%02x %02x%02x %02x%02x %02x%02x",
...
void SGB_WriteP1(u32 value)
{
    ...
    //    if(SGBInfo.delay > 0) return;
        ...
        if((p14^p15) == 0) //If both are 1 or 0...
        {
            SGBInfo.sending = 0;
            //Corrupted data...?
            Debug_DebugMsgArg("SGB warning - Unexpected operation. Corrupted SGB packet?");
            return;
        }
        /*
        if(p14 == 0)
