...
// TODO: add "dump all", "dump this" and "dump all mini" buttons
...
void Win_GB_GBCameraViewerUpdate(void)
{
    ...
    // If this is official GB Camera software
    // TODO: Make this check actually check for the official ROMs
    if (GameBoy.Emulator.RAM_Banks == 16)
    {
        GB_Debug_GBCameraMiniPhotoPrintAll(gb_allphotos_buffer);

        GUI_Draw_SetDrawingColor(255, 0, 0);
        int l = (gb_cameraview_selected_photo_index % 6) * (32 + 8) + 8 - 1;
        int t = (gb_cameraview_selected_photo_index / 6) * (32 + 8) + 8 - 1;
        int r = l + 32 + 1;
        int b = t + 32 + 1;
        GUI_Draw_Rect(gb_allphotos_buffer, GB_CAMERA_ALLPHOTOS_BUFFER_WIDTH,
                      GB_CAMERA_ALLPHOTOS_BUFFER_HEIGHT, l, r, t, b);
        l--;
        t--;
        r++;
        b++;
        ...
        GB_Debug_GBCameraPhotoPrint(gb_photo_zoomed_buffer, 128, 114,
                                    gb_cameraview_selected_photo_index);
    }

    //---------------------------

    GB_Debug_GBCameraWebcamOutputPrint(gb_camera_webcam_output_buffer,
                                       128, 112);
...
int Win_GB_GBCameraViewerCreate(void)
{
    ...
    GUI_SetBitmap(&gb_camview_photos_zoomed_bmp,
                  12 + GB_CAMERA_ALLPHOTOS_BUFFER_WIDTH + 6, 18, 128, 112,
    ...
    GUI_SetBitmap(&gb_camview_retina_processed_bmp, 412, 18 + 112 + 6, 128, 112,
                  gb_camera_retina_processed_buffer, NULL);

    GUI_SetBitmap(&gb_camview_scratch_photo_bmp, 412, 18 + 224 + 12, 128, 112,
                  gb_camera_scratch_photo_buffer, NULL);
    ...
    WinIDGBCameraViewer = WH_Create(WIN_GB_CAMERAVIEWER_WIDTH,
                                    WIN_GB_CAMERAVIEWER_HEIGHT, 0, 0, 0);
